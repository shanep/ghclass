...
const csv_individual = `"identifier","github_username","github_id","name"
...
test('individual student roster from csv', async (t) => {
    t.mock.method(fs, "readFile", async () => csv_individual)
...
test("individual construct repo url from student", (t) => {
    ...
    const expected = "git@github.com:shanep-classroom/project1-student1";
    assert.strictEqual(actual,expected);
})

const csv_group = `"identifier","github_username","github_id","name","group_name"
...
test('group student roster from csv', async (t) => {
    t.mock.method(fs, "readFile", async () => csv_group)
    const result = await ghclass.loadRosterCSV("test")
    assert.deepStrictEqual(
        result,
        [
...
test("group construct repo url from student", (t) => {
    const org = "shanep-classroom";
    const project = "project1"
    const student = {
        email: 'student1@u.boisestate.edu',
    ...
    const actual = ghclass.gitHubRepoUrlSSH(student, org, project);
    const expected = "git@github.com:shanep-classroom/project1-team-002";
    assert.strictEqual(actual,expected);
})
